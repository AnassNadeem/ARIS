...
def warmup_startup() -> dict[str, int]:
    ...
    for year in years:
        try:
            ...
            put_both(f"calendar_r2madrid26_{year}_now", cal, TTL_CALENDAR)
            calendars += 1
...
async def api_calendar(
    ...
    key = f"calendar_r2madrid26_{year}_{_as_of_key(as_of)}_{'replay' if replay else 'full'}"
    try:
        ...
        stale = cache.peek(key) or cache.peek(f"calendar_r2madrid26_{year}_now")
        if stale is not None:
            cal = stale
        else:
            try:
