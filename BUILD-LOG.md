...
- **2026-05-25 (Mon):** *Cloud deploy execution — the URL is live.* (Row reconstructed from the commit trail + the live app on May 30; it wasn't written on the calendar day, flagging that honestly per the no-backfill rule.) Wk 3 closed with the deploy still prep-only — `DEPLOY.md` + `scripts/deploy_to_neon.py` (`169227a`) were the runbook, this day was pressing the buttons. **Block 1–2 (Neon provisioning + remote schema/ingest):** created the Neon project, rewrote the dashboard connection string into the `postgresql+psycopg://…?sslmode=require` shape SQLAlchemy needs, and dropped it into a gitignored `.env.cloud`. The first connection attempts **failed** — the bare Neon compute endpoint wouldn't complete a psycopg handshake (the well-known Neon SNI quirk: a non-pooled endpoint needs either an `options=endpoint=<id>` parameter or the `-pooler` host). Rather than guess, wrote `scripts/diagnose_neon.py` (`0664ce2`) to probe five connection shapes in order — SQLAlchemy URL as-is, direct psycopg kwargs, kwargs + `options=endpoint=`, the `-pooler` host direct, and the pooler host via SQLAlchemy — plus a raw-TCP probe to separate "network blocked" from "TLS/handshake rejected." The pooler-host variant was the one that went green; switched `.env.cloud` to it. With the connection working, `deploy_to_neon.py` applied `db/schema.sql` to Neon and ingested the 2024 season **laps-only — telemetry deliberately skipped** (Neon free tier is 0.5 GB and Bahrain's 840 k telemetry rows alone are ~50 MB; full-season telemetry would blow the quota). **Block 3–4 (Streamlit Cloud + README):** deployed `apps/streamlit_app.py` on Streamlit Community Cloud against `main`, pasted the `ARIS_DB_URL` blob into the Secrets pane (the `st.secrets` shim from Wk 3 promotes it into the process env before `aris.io.db` imports). App is live at **https://aris-f1.streamlit.app** — season → race → driver → lap-time chart + MA(2) caption render end-to-end from Neon. Added the `**Live demo:**` line to the README and a `Live demo` row to the status table (`5339be9`). **The headline Wk 3 miss is closed: there is now a URL someone outside the laptop can open.** The browser-side steps (Neon project creation, Streamlit Cloud app creation + Secrets paste) are inherently manual; the scripted half (connection diagnosis, schema apply, season ingest) is captured in the two committed scripts so the deploy is reproducible. Telemetry-on-Neon and the sector chart are the remaining Phase 2 surface — Tue picks up the sector chart.
- **2026-05-26 (Tue):** *Sector breakdown chart + race-selector polish.* Each layer its own commit. **Block 1 (sector query):** added `fetch_lap_sectors(session_id, driver_id)` to `aris.io.db` — a column projection of the same `laps` rows `fetch_laps` already reads, returning `lap_number / sector_1_s / sector_2_s / sector_3_s` with the `NUMERIC` sectors cast to `float8` at the SQL boundary so the chart gets plain floats, not `Decimal`s. Unit test `test_fetch_lap_sectors_empty_has_right_columns` asserts a no-laps pair (`session_id=-1`) returns the four columns with zero rows rather than erroring — `pd.read_sql` keeps the cursor's column description even on an empty result, so the dashboard never has to guard against a `None`. Same commit extended `fetch_drivers` to carry the `team` column (`d135186`). Verified against the live local DB: Bahrain 2024 R drivers come back with teams (ALB→Williams, ALO→Aston Martin, BOT→Kick Sauber), sectors are `float64`, lap 1 `sector_1_s` is `NaN` (out-lap, as expected). **Block 2 (sector chart):** added a stacked-area `st.altair_chart` below the lap-time trace in `apps/streamlit_app.py` — `s1/s2/s3` melted to long form, dropped NaN sectors, stacked on `lap_number`, with an explicit blue/orange/green colour scale and a `labelExpr` that renders the legend as `S1/S2/S3` instead of the raw `sector_1_s` column names. Overlaid a dashed grey `mark_rule` at the driver's median lap time as the reference line, and a caption explaining the `S1+S2+S3 = lap time` sanity check and why the stacked total sits just under the median rule (rounding + in/out-lap delta). Defaulted to Altair because Streamlit ships it — no extra Cloud dependency. **Block 3 (race-selector polish):** the race dropdown already renders `R{round} — {country}` (Wk 3) and the season dropdown already defaults to newest (the `fetch_seasons` `ORDER BY year DESC` feeds `st.selectbox`, which picks index 0); verified both still hold. Driver dropdown now labels `VER — Max Verstappen (Red Bull)` via a `_driver_label` helper that falls back to `CODE — Full Name` when `team` is NULL. Tightened the empty-state messages so each carries a next step: "no races ingested for {season} — run `ingest_season.py {season}`", "no laps ingested for this race — try another round", "no sector times recorded for this driver." **Verification:** `ruff check` clean on the touched files (caught + fixed one E501 on the unpacking line); the Altair spec builds from real Bahrain data (167 long-form sector rows); `test_db` 2/2 green; app boots headless on `:8765` with `/_stcore/health → 200` and `GET / → 200`. Pushing `main` triggers the Streamlit Cloud auto-rebuild — the live-render confirmation in incognito is a browser step (the deployed app sits behind Streamlit's auth/wake redirect, so it can't be asserted headlessly from here). Commits: `d135186` (db helpers + test), `5f3134c` (app chart + polish).
- **2026-05-27 (Wed):** *`TrackStatus` filter + Wk2 script promotion + telemetry cleaning — the three engineering items Wk 2 and Wk 3 both flagged and deferred.* Each block its own commit; the SQL canary re-run at the end is the gate the whole day answers to. **Block 1 (green-only filter):** added a `TrackStatus == '1'` green-flag filter to the baseline path so SC / VSC / yellow / red-flag laps stop inflating the MAE. **Deliberate deviation from the plan's literal wording:** the plan said to add the filter inside `aris.eval.baseline.moving_average_baseline`, but that function takes an *already-filtered* frame (the recompute and cross-check paths both call `filter_clean_laps(detect_stints(...))` first) and its five unit tests build column-minimal frames with no `TrackStatus` column — requiring the column there would break them and split filtering across two functions. So the filter went into `aris.physics.stint.filter_clean_laps`, which is already the home for "drop laps that don't represent steady-state pace" (out-laps, in-laps, NaN times) — the same place, the same concern. It keys on the `TrackStatus` string and keeps only `'1'`; a multi-code status like `'24'` (yellow + SC during the lap) is correctly dropped, and the filter is skipped when the column is absent so synthetic frames and older callers are unaffected. Two new tests in `test_stint.py`: one builds a stint with a single SC lap (`TrackStatus='4'`) mid-stint and asserts it's gone from the clean set entirely (so it can neither feed the rolling average nor be scored), one asserts the back-compat skip when no column is present (`af1a57b`). **Block 2 (mirror in SQL):** added `AND track_status = '1'` to the `clean` CTE in `db/queries/baseline_ma2.sql` *and* to the dashboard's `fetch_driver_ma2_mae` window query in `aris.io.db` — so the deployed MA(2) caption reads the same filtered floor the canary checks, not a different number (`360ff91`). **Block 3 (regenerate + canary):** re-ran `scripts/recompute_wk2_baseline.py` through the now-green-only `filter_clean_laps`. Overall baseline MAE dropped from **1.088 s → 0.460 s** across 8 races (6734 → 6383 scored laps) — bigger than the Wk 2 retro's "~0.4 s" estimate. Per-race: Miami **2.484 → 0.413** (the red-flag race, exactly as predicted), Australia 1.698 → 0.486, Saudi 1.169 → 0.489, Bahrain 0.373 → 0.328, '23 Bahrain 1.072 → 0.339; Abu Dhabi unchanged at 0.382 (no non-green laps in its scored set). Then re-ran `scripts/baseline_crosscheck.py`: the SQL-from-Postgres baseline and the pandas-from-FastF1 baseline **still match at machine epsilon — max abs diff 6.66e-16 s across all 8 races** — proving the filter is applied identically on both sides and the ingest is still lossless. The 0.460 s number is the new floor Phase 3 must beat (`4b3b79e`, csv footer updated). **Block 4 (promote the script):** moved `scripts/wk2_day3_block4.py` into the package as `src/aris/eval/run_baseline_all_races.py` with a `python -m aris.eval.run_baseline_all_races` entry point (and a `--window` flag, default 2). The promoted module imports the canonical `detect_stints` / `filter_clean_laps` / `moving_average_baseline` / scoring functions instead of the inline copies the old dry-run script carried, so the runner can never drift from the rest of ARIS; deleted the old script. Verified `python -m aris.eval.run_baseline_all_races` reproduces the green-only floor exactly (0.459772 s, 6383 laps). This rename was flagged in both the Wk 2 and Wk 3 retros — finally done (`c8f2280`). **Block 5 (telemetry cleaning):** `aris.io.ingest._clean_gear` now nulls any gear outside `1..8` at the ingest boundary, and the `telemetry` upsert's `ON CONFLICT` was switched from `DO NOTHING` to `DO UPDATE SET gear = EXCLUDED.gear` for that *one* column — so re-ingesting cleans gears stored before the rule existed while every other channel stays immutable. Honest correction to the Wk 3 estimate: the spot-check said "~20 garbage-gear samples," but querying the real table found **420** — the original eyeball only caught the high outliers (47, 17, 75) and missed `gear = 0` (neutral/unknown) plus the spread of mid-range glitches (9–75). Re-ingested Bahrain 2024 R telemetry: net `+0` rows (idempotent — all 840 934 samples already present), but the `DO UPDATE` nulled all 420 bad gears in place. Invariant now holds: `SELECT count(*) FROM telemetry WHERE gear < 1 OR gear > 8 = 0`, gear range is `1..8`, the 420 are NULL, and **no rows were lost** (840 934 intact). Tests: `test_telemetry_clean.py` unit-tests `_clean_gear` (1–8 pass, 0/-1/9/17/47/75 → None, NaN/None → None, runs in CI without a DB); `test_ingest.py` gained the DB-invariant integration test (`baa04eb`); `db/SCHEMA-NOTES.md` documents the rule. **Verification:** full suite **green** (was 34, now 54 — +2 db, +2 stint, +16 gear-unit, +1 telemetry-invariant), `ruff check` clean on every touched file, canary green at the new 0.460 s floor. Pushing `main` triggers the Streamlit Cloud rebuild — the deployed MA(2) caption now reads the filtered floor. Commits: `af1a57b` (filter), `360ff91` (SQL mirror), `4b3b79e` (regen csv), `c8f2280` (promote), `baa04eb` (telemetry clean).
- **2026-05-28 (Thu):** *README hero rewrite + status-table bump + `v0.2-pipeline` tag — the repo's front page now presents Phase 2, not Phase 1.* **Block 1 (screenshot pass) — honest miss, flagged not faked:** the deliverable is a deployed-dashboard screenshot for the hero, and it isn't captured. Two compounding blockers: the live app sits behind Streamlit Cloud's wake/auth redirect (can't be screenshotted headlessly from here, same constraint noted Day 2), and the local fallback — run the app against Neon and screenshot localhost — died on a broken venv (`uv sync` kept hitting `Access is denied` removing `*.dist-info` dirs under OneDrive's file lock, so `streamlit`'s console script never installed and `streamlit.web.cli` is half-present). Neon itself is reachable (`SELECT count(*)` → 26 604 laps / 24 sessions), so the data path is fine; it's purely the local UI-render + capture step that's a manual browser action. **Decision: do not fabricate a UI screenshot.** Instead restructured the hero honestly — top of README now *leads* with the live-demo callout (URL + one-paragraph "what's behind it"), and the dashboard PNG is marked as a pending in-browser capture with an HTML comment pointing at this log row. **Block 2 (hero swap):** the Phase-1 speed-vs-distance VER chart was demoted, not deleted — it moved into a new "Phase 1 artefact — first FastF1 pull" section lower down with a caption that frames it as where the project started (`6a7216c`). **Block 3 (pitch rewrite):** added a "Where it stands today (Phase 2 complete)" paragraph — idempotent FastF1 → Postgres ingest, MA(2) baseline per stint, public Streamlit surface, the dual pandas/SQL cross-check at machine epsilon as the lossless-ingest canary, and the 0.460 s green-flag floor as the number Phase 3 must beat (`1e76e50`). **Block 4 (status table + roadmap):** `Current phase` → "Phase 2 complete — Phase 3 (lap-time predictor) opens Jun 1"; `Last tag` → `v0.2-pipeline` with the corrected **0.460 s MAE / 6383 laps green-only** floor (was the stale `v0.1` line at 1.088 s / 6734 laps); roadmap rows marked `v0.1-foundation` ✅ and `v0.2-pipeline` ✅, Phase 3 `◀ next` (`00eb3cc`). **Block 5 (canary re-run + tag):** ran `scripts/baseline_crosscheck.py` against the **live Neon DB** — all 5 ingested 2024 races reproduce the Wk 2 pandas baseline at machine epsilon (**max abs diff 8.33e-16 s**: Bahrain 5.55e-17, Saudi 8.33e-16, Australia 3.89e-16, Japan 5.55e-16, Miami 4.44e-16), lap counts identical. The script exits 1 only because the three 2023 reference races aren't on Neon (the Day 1 ingest was 2024-only — Neon free tier is 0.5 GB), not because of any mismatch. The full 8-race canary's last green run is Day 3's local-DB pass at 6.66e-16 s; nothing in Day 4 touched the baseline path (README-only commits), so the tagged commit's canary state is unchanged and now additionally corroborated against the deployed data. Tagged `v0.2-pipeline` (annotated) on `00eb3cc` — URL live ✓, sector chart deployed (Day 2) ✓, canary green ✓, README hero reframed ✓ — and pushed the tag + `main`. **Verification:** `git tag` shows `v0.1-foundation` + `v0.2-pipeline`; both on `origin`. The one open thread into Fri/Sat: the real dashboard screenshot for the hero + LinkedIn, which needs a working browser capture. Commits: `6a7216c`, `1e76e50`, `00eb3cc`; tag `v0.2-pipeline`.
- **2026-05-29 (Fri):** *Hardening + Phase 3 prep + release notes — closing the loose ends before Sat's post.* **Block 1 (cold-clone sanity):** ran the full unit suite through the venv interpreter — **46 passed, 6 skipped** (the `test_db` / `test_ingest` integration cases skip cleanly with no `ARIS_DB_URL`, which is exactly CI's state). The genuine cold-clone trap this surfaced: the tracked root `requirements.txt` was **empty** — a cold-cloner running `pip install -r requirements.txt` got a no-op and then a broken import. Populated it as a no-uv pip fallback mirroring `pyproject.toml`'s runtime + the `pytest`/`ruff` dev subset, with a header pointing uv users at `uv sync` and noting Streamlit Cloud reads its own `apps/requirements.txt` (`51f380e`). Honest caveat: I did *not* do the literal clone-into-a-fresh-temp-dir install, because the working venv is still wedged by the OneDrive file-lock issue from Day 4 (`uv sync` can't remove `*.dist-info` to rebuild) — the green unit suite plus the dependency-file audit is the proxy, and the fix is real regardless of how it was found. **Block 2 (GitHub releases):** drafted release notes for both `v0.1-foundation` (Phase 1) and `v0.2-pipeline` (Phase 2) — titles + bodies linking the respective retros and the live URL, v0.1 attaching `assets/screenshots/wk2-bahrain-stints.png`. The `gh release create` calls are outward-facing publishing, so they're staged to run at week close rather than fired mid-task; the tags themselves are already on `origin`. **Block 3 (Phase 3 entry-point sketch):** wrote `docs/planning/PHASE3-ENTRY-SKETCH.md` (local-only — `docs/` is gitignored) specifying the Wk 5 Day 1 leakage tripwire: the test name (`test_leakage_tripwire.py::test_no_feature_sees_its_own_or_future_lap`), the synthetic one-driver/one-stint frame with a unique-per-lap seed as the tripwire signal, the three assertions (feature support ⊂ laps `< N`; shuffling future rows doesn't move lap N's features; MA(k) baseline passes as a regression guard), and the failure modes it catches (missing `shift(1)`, stint-key bleed across a pit stop, global normalisation fit on test laps). Spec only, no implementation. **Block 4 (README final pass):** rewrote the stale "Getting started (Phase 1)" — now a phase-agnostic "Getting started" with both the `uv sync --extra dev` path and the plain-pip path, a "Run the dashboard locally (Phase 2)" block (`docker compose up` → `ingest_season.py 2024` → `streamlit run`), and a "Replay the baseline cross-check" block surfacing `baseline_crosscheck.py` + `python -m aris.eval.run_baseline_all_races`. Also dropped the repo-layout line claiming `tests/` includes "the leakage tripwire" — that test doesn't exist until Wk 5, and a recruiter who greps for it shouldn't find a phantom (`4ac971d`). **Block 5 (close):** Sat's LinkedIn prerequisites audited — URL live ✓, CI green on `main` ✓, README reframed around the dashboard ✓; the single gap is the real deployed-dashboard screenshot, which is *also* the post's hero attachment, so the hero swap and the LinkedIn image ride on the same one pending in-browser capture. Commits: `51f380e`, `4ac971d`.
- **2026-05-30 (Sat):** *Phase 2 closes — Wk 4 + Phase 2 retros, Wk 5 plan, LinkedIn gate.* **Block 1 (URL sanity):** the data path behind the URL is verified — the Day 4 canary reproduced all 5 ingested 2024 races against the live Neon DB at machine epsilon. The deployed-page render in incognito/mobile is a browser check that can't be asserted headlessly from here (Streamlit Cloud wake/auth redirect), so it stays a manual confirmation, flagged not faked. **Block 2 (retros):** wrote the Wk 4 weekly retro + the **Phase 2 retrospective** below — Phase 2's empty retrospective box is now filled (tag, live URL, what I'd change). **Block 3 (Wk 5 plan):** appended the full Wk 5 detailed plan to `ARIS-WEEKLY-DETAILED-PLANS.md` (local-only — `docs/` is gitignored) — Phase 3 opens with the leakage tripwire as Day 1's only real commit, *then* the hand-coded bicycle model, *then* the Brunel WhatsApp (URL + a physics layer now both exist to lead with). **Block 4 (LinkedIn gate):** the four-criterion gate passes **3 of 4** — URL live ✓, CI green ✓, body final ✓, but criterion 3 (README hero is a *deployed-dashboard screenshot*) fails because that capture is a pending in-browser step. Per the standing rule, **deferred to Wk 5 Day 3** with the reason logged in `docs/social/linkedin-post-01-final.md`; also corrected that post's stale baseline number (1.1 s → **0.460 s** green-flag floor) and dropped in the live URL so it's publish-ready the moment the screenshot lands. The penalty for waiting one week is zero — exactly what the gate was designed for. **Block 5 (open obligations):** `ARIS-Tracker.xlsx` Wk 3 + Wk 4 rows still need the manual binary edit (carried, noted); `gh issue list` sweep deferred with the GitHub release creation to the week-close network window. **Block 6 (close):** `v0.2-pipeline` shipped, the floor Phase 3 must beat is **0.460 s**, Phase 2 is done. **Sun May 31 is off.**
- **2026-05-31 (Sun):** *off*
